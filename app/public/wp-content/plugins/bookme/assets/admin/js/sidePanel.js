...
var bm_show_sidepanel = function ($panel) {
...
var bm_hide_sidepanel = function ($panel) {
    if($panel){
...
jQuery(document).on("click", ".slidePanel-close", function (e) {
    bm_hide_sidepanel(jQuery(this).parents('.slidePanel'));
}).on("click", ".slidePanel-wrapper", function (e) {
    bm_hide_sidepanel(lastPanel[lastPanel.length - 1]);
});