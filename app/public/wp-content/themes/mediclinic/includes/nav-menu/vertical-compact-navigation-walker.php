...
/**
 * Custom WP_NAV_MENU function for vertical compact navigation
 */
if(!class_exists('MediclinicMikadoVerticalCompactNavigationWalker')) {
	class MediclinicMikadoVerticalCompactNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl(&$output, $depth = 0, $args = array()) {
			...
			if($this->background_image !== '') {
				...
				$background_image_class = ' mkdf-dropdown-with-background-image';
			}

			$indent = str_repeat("\t", $depth);
			if($depth == 0) {
				$out_div = '<div class="second '.$background_image_class.'" '.mediclinic_mikado_get_inline_style($menu_image_background).'><div class="inner">';
			} else {
				$out_div = '';
			}
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
			...
			if(!empty($item->icon)) {
				$class_names .= ' mkdf-menu-item-with-icon';
			}

			if(!empty($item->description)) {
				$class_names .= ' mkdf-menu-item-with-description';
			}
