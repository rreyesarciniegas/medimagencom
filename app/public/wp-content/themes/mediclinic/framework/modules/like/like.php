...
class MediclinicMikadoLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_mediclinic_mikado_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_mediclinic_mikado_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'mediclinic' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'mediclinic' );
		}
...
if ( ! function_exists( 'mediclinic_mikado_create_like' ) ) {
	function mediclinic_mikado_create_like() {
		MediclinicMikadoLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'mediclinic_mikado_create_like' );
}