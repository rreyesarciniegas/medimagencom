...
if ( ! function_exists( 'mediclinic_mikado_get_hide_dep_for_header_logo_area_meta_boxes' ) ) {
	function mediclinic_mikado_get_hide_dep_for_header_logo_area_meta_boxes() {
		$hide_dep_options = apply_filters( 'mediclinic_mikado_header_logo_area_hide_meta_boxes', $hide_dep_options = array() );
		
		return $hide_dep_options;
	}
}

if ( ! function_exists( 'mediclinic_mikado_header_logo_area_meta_options_map' ) ) {
	function mediclinic_mikado_header_logo_area_meta_options_map( $header_meta_box ) {
		$hide_dep_options = mediclinic_mikado_get_hide_dep_for_header_logo_area_meta_boxes();
		...
		mediclinic_mikado_add_admin_section_title(
			array(
				...
				'title'  => esc_html__( 'Logo Area Style', 'mediclinic' )
			)
		);
		...
		mediclinic_mikado_create_meta_box_field(
			array(
				...
				'type'          => 'yesno',
				'default_value' => 'no',
				'label'         => esc_html__( 'Disable Header Logo Area Widget', 'mediclinic' ),
				'description'   => esc_html__( 'Enabling this option will hide widget area from the logo area', 'mediclinic' ),
				'parent'        => $logo_area_container
			)
		);
		...
		if ( count( $mediclinic_custom_sidebars ) > 0 ) {
			mediclinic_mikado_create_meta_box_field(
				array(
					...
					'type'        => 'selectblank',
					'label'       => esc_html__( 'Choose Custom Widget Area for Logo Area', 'mediclinic' ),
					'description' => esc_html__( 'Choose custom widget area to display in header logo area"', 'mediclinic' ),
					'parent'      => $logo_area_container,
		...
		mediclinic_mikado_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Set grid background transparency for logo area (0 = fully transparent, 1 = opaque)', 'mediclinic' ),
				'parent'      => $logo_area_in_grid_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		mediclinic_mikado_create_meta_box_field(
			array(
				...
				'description'   => esc_html__( 'Set border on grid logo area', 'mediclinic' ),
				'parent'        => $logo_area_in_grid_container,
				'default_value' => '',
				'options'       => mediclinic_mikado_get_yes_no_select_array(),
				'args'          => array(
		...
		mediclinic_mikado_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a background color for logo area', 'mediclinic' ),
				'parent'      => $logo_area_container
			)
		);
		...
		mediclinic_mikado_create_meta_box_field(
			array(
				...
				'description' => esc_html__( 'Choose a transparency for the logo area background color (0 = fully transparent, 1 = opaque)', 'mediclinic' ),
				'parent'      => $logo_area_container,
				'args'        => array(
					'col_width' => 2
				)
		...
		$logo_area_border_bottom_color_container = mediclinic_mikado_add_admin_container(
			array(
				'type'            => 'container',
		...
		do_action( 'mediclinic_mikado_header_logo_area_additional_meta_boxes_map', $logo_area_container );
	}
	
	add_action( 'mediclinic_mikado_header_logo_area_meta_boxes_map', 'mediclinic_mikado_header_logo_area_meta_options_map', 10, 1 );
}