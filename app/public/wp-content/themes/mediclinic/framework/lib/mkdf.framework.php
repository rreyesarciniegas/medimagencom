...
/*
   Class: MediclinicMikadoFramework
...
class MediclinicMikadoFramework
{
    ...
    private function __construct() {
        $this->mkdOptions = MediclinicMikadoOptions::get_instance();
        $this->mkdMetaBoxes = MediclinicMikadoMetaBoxes::get_instance();
    }
    ...
    public function setSkin(MediclinicMikadoSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class MediclinicMikadoSkinManager
 *
...
class MediclinicMikadoSkinManager
{
    ...
    /**
     * @see MediclinicMikadoSkinManager::setSkin()
     */
    ...
    private function setSkin($skinName = 'mikado') {
        if ($skinName !== '') {
            if (file_exists(get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php')) {
				...
				$this->skin = new MediclinicMikadoSkin();
            }
        } else {
            $this->skin = false;
        }
...
/**
 * Class MediclinicMikadoSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class MediclinicMikadoSkinAbstract
{
    /**
     * @var string
     */
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if ($template !== '') {
            include(mediclinic_mikado_find_template_path('framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.mkdSCIcon = '<?php echo mediclinic_mikado_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class MediclinicMikadoOptions
{
    ...
    public function getOptionValue($key) {
        ...
        if (array_key_exists($key, $mediclinic_mikado_options)) {
            return $mediclinic_mikado_options[$key];
...
/*
   Class: MediclinicMikadoAdminPage
   A class that initializes Mikado Admin Page
*/

class MediclinicMikadoAdminPage implements iMediclinicMikadoLayoutNode
{
    ...
    function __construct($slug = "", $title = "", $icon = "") {
        $this->layout = array();
        $this->factory = new MediclinicMikadoFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iMediclinicMikadoRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: MediclinicMikadoMetaBoxes
...
/*
   Class: MediclinicMikadoMetaBox
   A class that initializes Mikado Meta Box
*/

class MediclinicMikadoMetaBox implements iMediclinicMikadoLayoutNode
{
    ...
    public function setTitle($label) {
        ...
        return $mediclinic_mikado_Framework->getSkin()->getSkinLabel() . ' ' . $label;
    }
}

if (!function_exists('mediclinic_mikado_init_framework_variable')) {
    function mediclinic_mikado_init_framework_variable() {
        global $mediclinic_mikado_Framework;

        $mediclinic_mikado_Framework = MediclinicMikadoFramework::get_instance();
        $mkdSkinManager = new MediclinicMikadoSkinManager();
        $mediclinic_mikado_Framework->setSkin($mkdSkinManager->getSkin());
    }

    add_action('mediclinic_mikado_before_options_map', 'mediclinic_mikado_init_framework_variable');
}
?>