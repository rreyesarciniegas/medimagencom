(function($){
	var MikadoSidebar = function(){
		...
		this.widget_add  = $('#mkd-add-widget');
	...
	MikadoSidebar.prototype = {
		...
		create_form: function(){
			...
			this.widget_name = this.widget_wrap.find('input[name="mkd-sidebar-widgets"]');
			this.nonce = this.widget_wrap.find('input[name="mkd-delete-sidebar"]').val();   
		},

		add_del_button: function(){
			this.widget_area.find('.sidebar-mkd-custom').append('<span class="mkd-delete-button"></span>');
		},

		bind_events: function(){
			this.widget_wrap.on('click', '.mkd-delete-button', $.proxy( this.delete_sidebar, this));
		},
