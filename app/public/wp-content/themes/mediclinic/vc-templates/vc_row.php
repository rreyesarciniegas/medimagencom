...
$scrolling_text_structure = '';

if($scrolling_text != '') {

	$scrolling_text_style = array();

	if($scrolling_text_color !== ''){
		$scrolling_text_style[] = 'color:'.  $scrolling_text_color;
	}
	if($scrolling_text_font_size !== ''){
		$scrolling_text_style[] = 'font-size:'.  $scrolling_text_font_size. 'px';
	}
	if($scrolling_text_font_family !== ''){
		$scrolling_text_style[] = 'font-family:'.  $scrolling_text_font_family;
	}
	if($scrolling_text_font_weight !== ''){
		$scrolling_text_style[] = 'font-weight:'.  $scrolling_text_font_weight;
	}
	if($scrolling_text_letter_spacing !== ''){
		$scrolling_text_style[] = 'letter-spacing:'.  $scrolling_text_letter_spacing . 'px';
	}
	if($scrolling_text_text_transform !== ''){
		$scrolling_text_style[] = 'text-transform:'.  $scrolling_text_text_transform;
	}
	if($scrolling_text_font_style !== ''){
		$scrolling_text_style[] = 'font-style:'.  $scrolling_text_font_style;
	}
	if($scrolling_text_top_position !== ''){
		$scrolling_text_style[] = 'top:'.  $scrolling_text_top_position;
	}

	$scrolling_text_html = '';
	$scrolling_text_html .= '<div class="mkdf-marquee-holder" '. mediclinic_mikado_get_inline_style(implode(';', $scrolling_text_style)) .' ><ul>';
	$scrolling_text_html .= '<li class="mkdf-marquee">';
	$scrolling_text_html .= $scrolling_text;
	$scrolling_text_html .= '</li>';
	$scrolling_text_html .= '<li class="mkdf-marquee">';
	$scrolling_text_html .= $scrolling_text;
	$scrolling_text_html .= '</li>';
	$scrolling_text_html .= '</ul></div>';

	$scrolling_text_structure = $scrolling_text_html;

	$css_classes[] = 'mkdf-scrolling-text-enabled';
}
...
if($row_content_width === 'grid'){
	$mkdf_row_wrapper_start .= '<div class="mkdf-row-grid-section-wrapper '.esc_attr($grid_row_classes).'" '.esc_attr($grid_row_datas).' ' . mediclinic_mikado_get_inline_style( $mkdf_grid_row_styles ) . '><div class="mkdf-row-grid-section">';
...
$output .= '<div ' . implode( ' ', $wrapper_attributes ) . ' ' . mediclinic_mikado_get_inline_style( $mkdf_vc_row_styles ) . '>';
$output .= $scrolling_text_structure;
$output .= wpb_js_remove_wpautop( $content );
$output .= '</div>';
$output .= $mkdf_row_wrapper_end;
$output .= $after_output;

echo mediclinic_mikado_get_module_part( $output );
